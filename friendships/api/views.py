...
class FriendshipViewSet(viewsets.GenericViewSet):
    ...
    def follow(self, request, pk):
        ...
        if Friendship.objects.filter(from_user=request.user, to_user=pk).exists():
            return Response({
                'success': False,
                'duplicate': 'you have followed the user, can not do',
    ...
    def unfollow(self, request, pk):
        ...
        if not Friendship.objects.filter(from_user=request.user, to_user=unfollow_user).exists():
            return Response({
                'success': False,
                'duplicate': 'you have not followed the user, can not unfollow',
            }, status=status.HTTP_400_BAD_REQUEST)
